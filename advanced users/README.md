...
11.	When the csi-vxflex-install playbook has successfully finished, next step is to log in to the Kubernetes Master host <br>Switch from root to your user <br>Run this command: #Once that is complete run this command on your Kubernetes Master host: cd ../dell-csi-helm-installer && ./csi-install.sh --namespace vxflexos --values ../helm/myvalues.yaml --skip-verify-node <br>
